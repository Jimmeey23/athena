...
export const CONTEXT_TEMPLATES: ContextTemplate[] = [
  {
    ...
    fields: [
      { id: 'partnerName', label: 'Host name', type: 'text', required: true },
      { id: 'classType', label: 'Class details / session', type: 'select', required: true },
      { id: 'studio', label: 'Studio name', type: 'select', required: true },
      { id: 'memberFeedback', label: 'Attendee details with comments', type: 'textarea', required: true, placeholder: 'Summarise each attendee response and include comments captured from the session.' },
      ...
      { id: 'followUpPlan', label: 'Collaborate again in future?', type: 'select', required: true, options: ['Definitely collaborate again', 'Maybe collaborate again', 'Do not collaborate again', 'Needs further review', 'Unable to determine'] },
...
export function buildHostedClassFeedbackText(input: HostedClassFeedbackInput): string {
  ...
  return [
    ...
    input.partnerName ? `Host name: ${input.partnerName}` : '',
    ...
    input.followUpPlan ? `Collaborate again in future: ${input.followUpPlan}` : '',
